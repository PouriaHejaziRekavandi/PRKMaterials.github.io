...
net_dir = os.path.join(save_dir, 'trained_net_dir')
os.makedirs(net_dir, exist_ok=True)
keras_model_path = os.path.join(net_dir, 'model.keras')
pkl_path = os.path.join(net_dir, 'net_instance.pkl')
...
if os.path.exists(pkl_path) and os.path.exists(keras_model_path):
    print("✅ Found trained network on Google Drive! Loading it now...")
    # Load the Net object (containing internal states, scalers, etc.)
    with open(pkl_path, 'rb') as f:
        net = joblib.load(f)
    # Re-attach the Keras model natively
    net.model = tf.keras.models.load_model(keras_model_path)
else:
    ...
    # Save the Keras model natively
    net.model.save(keras_model_path)
    # Temporarily detach the model to avoid pickling Keras graphs
    model_backup = net.model
    net.model = None
    with open(pkl_path, 'wb') as f:
        joblib.dump(net, f)
    # Re-attach
    net.model = model_backup
