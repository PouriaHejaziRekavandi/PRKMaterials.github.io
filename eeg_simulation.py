...
def initialize_pipeline():
    ...
    # Optimize CSR matrix row extraction using NumPy split
    neighbors = [np.array(x) for x in np.split(adj.indices, adj.indptr[1:-1])]
...
def evaluate_model(net, fwd_test, info_test, pos_gm, neighbors):
    ...
    return y_true_extracted_l, y_pred_extracted_l, mle_l, found_l, auc_l, mse_l, nmse_l
...
def main():
    ...
    y_true_extracted_l, y_pred_extracted_l, mle_l, found_l, auc_l, mse_l, nmse_l = evaluate_model(
        net, fwd_test, info_test, pos_gm, neighbors
    )

    plot_results(all_loss, all_val_loss, mle_l)

    save_and_load_data(y_true_extracted_l, y_pred_extracted_l, mle_l, mse_l, nmse_l, auc_l, found_l)
