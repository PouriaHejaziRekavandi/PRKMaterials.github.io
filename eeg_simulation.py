...
def train_model(net, fwd, info, sim_settings, checkpoints_dir, total_samples=TOTAL_SAMPLES, chunk_size=CHUNK_SIZE):
    ...
    for chunk_idx in range(num_chunks):
        ...
        # esinet.Net returns itself and does not expose keras history natively,
        # so we monkey-patch the inner model's fit method to inject a LambdaCallback.
        original_fit = net.model.fit

        def monkey_fit(*args, **kwargs):
