...
def setup_environment(drive_path='/content/drive/MyDrive/esinet_project'):
    """
    Mounts Google Drive and prepares the save directory.
    """
    try:
        from google.colab import drive
        drive.mount('/content/drive')
        save_dir = drive_path
        logging.info(f"✅ Google Drive mounted. Files will be saved to: {save_dir}")
    except Exception:
        save_dir = 'esinet_project_local'
        logging.info(f"ℹ️ Colab Drive not found, using local directory: {save_dir}")
...
def train_incremental(fwd, info, settings, save_dir, total_samples=5000, chunk_size=1000):
    """
    ...
    for i in range(num_chunks):
        ...
        if os.path.exists(sim_chunk_path):
            logging.info(f"✅ Loading chunk {i+1} from Drive...")
            ...
            logging.info(f"💾 Chunk {i+1} saved to Drive.")

        # Fit
        history = net.fit(sim, epochs=10, validation_split=0.1)
        if hasattr(history, 'history'):
            history_accumulator['loss'].extend(history.history.get('loss', []))
            history_accumulator['val_loss'].extend(history.history.get('val_loss', []))
...
def main():
    ...
    net, history = train_incremental(fwd_train, info, custom_settings, save_dir, total_samples=5000, chunk_size=1000)
