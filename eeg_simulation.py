...
def initialize_pipeline():
    ...
    # Vectorized adjacency indices extraction
...
def train_model(net, fwd, info, sim_settings, checkpoints_dir, total_samples=TOTAL_SAMPLES, chunk_size=CHUNK_SIZE):
    ...
    for chunk_idx in range(num_chunks):
        ...
        history = net.fit(sim_train, epochs=EPOCHS, batch_size=BATCH_SIZE, validation_split=0.1)
...
def evaluate_model(net, fwd_test, info_test, sim_settings, pos_gm, neighbors):
    ...
    for i in range(len(y_true)):
        ...
        y_true_extracted.append(jt)
        y_pred_extracted.append(jp)
    ...
    return y_true_extracted, y_pred_extracted, mle_l, found_l, auc_l, mse_l, nmse_l
...
def save_and_load_data(y_true_extracted, y_pred_extracted, mle_l, mse_l, nmse_l, auc_l, found_l):
    ...
    np.savez_compressed(
        save_path,
        y_true_peak=np.array(y_true_extracted, dtype=float),
        y_pred_peak=np.array(y_pred_extracted, dtype=float),
        mle=np.array(mle_l),
        mse=np.array(mse_l),
...
def main():
    ...
    y_true_extracted, y_pred_extracted, mle_l, found_l, auc_l, mse_l, nmse_l = evaluate_model(
        net, fwd_test, info_test, sim_settings, pos_gm, neighbors
    )

    plot_results(all_loss, all_val_loss, mle_l)

    save_and_load_data(y_true_extracted, y_pred_extracted, mle_l, mse_l, nmse_l, auc_l, found_l)
